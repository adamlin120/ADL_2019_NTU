...
class DialogDataset(Dataset):
    ...
    def __init__(self, data, padding=0,
                 ...
                 context_padded_len=300, option_padded_len=50, shuffle=False):
        self.data = data
    ...
    def __getitem__(self, index):
        ...
        if self.n_negative == -1:
            ...
            positive_indices = random.sample(list(range(len(positives))), n_positive)
        ...
        data['speaker_option'] = data['speaker'][-2]
        data['speaker'] = [[spk]*len(data['context'][i]) for i, spk in enumerate(data['speaker'])]
        data['speaker'] = list(itertools.chain.from_iterable(data['speaker']))
        data['context'] = list(itertools.chain.from_iterable(data['context']))
        if len(data['context']) > self.context_padded_len:
            data['context'] = data['context'][-self.context_padded_len:]
            data['speaker'] = data['speaker'][-self.context_padded_len:]
    ...
    def collate_fn(self, datas):
        ...
        batch['context_lens'] = torch.tensor(batch['context_lens'],
                                             dtype=torch.int64)
        batch['speaker'] = [pad_to_len(data['speaker'], padded_len, -1)
                            for data in datas]
        ...
        batch['option_lens'] = torch.tensor(batch['option_lens'],
                                            dtype=torch.int64)
...
def pad_to_len(arr, padded_len, padding=0):
    ...
    if len(arr) > padded_len:
        ...
        arr += [padding] * (padded_len - len(arr))
    return arr
