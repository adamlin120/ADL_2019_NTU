...
class Preprocessor:
    ...
    def __init__(self, embedding):
        ...
        self.tokenizer = spacy.load('en', disable=['textcat', 'tagger',
                                                  'ner', 'parser'])
        self.tokenizer.max_length = int(1e9)
    ...
    def tokenize(self, sentence):
        ...
        """
        indices = [token.text for token in self.tokenizer(sentence)]
        return indices
    ...
    def sentence_to_indices(self, sentence):
        ...
        if self.embedding is None:
            raise TypeError(f"embedding: {self.embedding} is not initialized!")
        indices = [self.embedding.to_index(word)
                   for word in self.tokenize(sentence.lower())]
        if len(indices) == 0:
            indices = [self.embedding.to_index('<unk>')]
        return indices
    ...
    def preprocess_sample(self, data):
        ...
        for message in data['messages-so-far']:
            one_sent = self.sentence_to_indices(message['utterance'].lower())
            # append content
            processed['context'].append(one_sent)
            # append speaker
            processed['speaker'].append(message['speaker'].lower())
