    ...
    def fit_dataset(self, data, collate_fn=default_collate, callbacks=[]):
        ...
        while self.epoch < self.max_epochs:
            ...
            loader = torch.utils.data.DataLoader(data, batch_size=self.batch_size, shuffle=True,
                                                 num_workers=self.n_works,
                                                 collate_fn=collate_fn,
                                                 pin_memory=True)
            # train epoch
            log_train = self._run_epoch(loader, True)
            ...
            if self.valid is not None:
                ...
                # and `collate_fn=collate_fn`.
                # evaluate model
                loader = torch.utils.data.DataLoader(self.valid,
                                                     batch_size=self.batch_size,
                                                     num_workers=self.n_works,
                                                     collate_fn=collate_fn,
                                                     pin_memory=True)
                log_valid = self._run_epoch(loader, False)
            else:
                log_valid = None
...
#         self.epoch = checkpoint['epoch']
    ...
    def _run_epoch(self, loader, training):
        ...
        if training:
            iter_in_epoch = min(len(loader), self.max_iters_in_epoch)
